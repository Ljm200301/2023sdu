...
class CryptSM2(object):
    ...
    def set_key(self,publickey,privatekey):
    ...
    def _sub_point(self, P1, P2):
        ...
        if (l1 < len_2) or (l2 < len_2):
            ...
            # 转换为 Jacobian 坐标系
            X1 = (X1 * Z2 * Z2) % int(self.ecc_table['p'], base=16)
            Y1 = (Y1 * Z2 * Z2 * Z2) % int(self.ecc_table['p'], base=16)
            Z1 = (Z1 * Z2) % int(self.ecc_table['p'], base=16)
            X2 = (X2 * Z1 * Z1) % int(self.ecc_table['p'], base=16)
            Y2 = (Y2 * Z1 * Z1 * Z1) % int(self.ecc_table['p'], base=16)
            ...
            if X1 == X2:
                ...
                T1 = (Y1 - Y2) % int(self.ecc_table['p'], base=16)
                T2 = (X1 - X2) % int(self.ecc_table['p'], base=16)
                T3 = (X1 * Y2 - X2 * Y1) % int(self.ecc_table['p'], base=16)
                T4 = (T2 * T2) % int(self.ecc_table['p'], base=16)
                T5 = (T2 * T4) % int(self.ecc_table['p'], base=16)
                T6 = (T1 * T1 * Z1 * Z2 - T4 * T3) % int(self.ecc_table['p'], base=16)
                X3 = (T1 * T5) % int(self.ecc_table['p'], base=16)
                Y3 = (T3 * T5 - T6 * T2) % int(self.ecc_table['p'], base=16)
                Z3 = (T5 * Z1 * Z2) % int(self.ecc_table['p'], base=16)
                ...
                if Z3 == 1:
                    ...
                    inv_Z3 = pow(Z3, -1, int(self.ecc_table['p'], base=16))
                    X3 = (X3 * inv_Z3 * inv_Z3) % int(self.ecc_table['p'], base=16)
                    Y3 = (Y3 * inv_Z3 * inv_Z3 * inv_Z3) % int(self.ecc_table['p'], base=16)
                    form = '%%0%dx' % self.para_len
                    form = form * 3
                    return form % (X3, Y3, inv_Z3)


    def sm2_point_subtraction(self, P1, P2):
        if P1 == P2:
            # 如果 P1 和 P2 相等，则返回无穷远点
            return self.infinity

        x1, y1 = self.hex_to_int(P1[:self.para_len]), self.hex_to_int(P1[self.para_len:])
        x2, y2 = self.hex_to_int(P2[:self.para_len]), self.hex_to_int(P2[self.para_len:])
        ...
        if y1 == y2:
            # 如果 P1 和 P2 的坐标相同，则返回无穷远点
            return self.infinity

        p = self.hex_to_int(self.ecc_table['p'])
        k = (y1 - y2) * self.mod_inverse(x1 - x2, p)
        x3 = (k * k - x1 - x2) % p
        y3 = (k * (x1 - x3) - y1) % p

        return self.int_to_hex(x3, self.para_len) + self.int_to_hex(y3, self.para_len)
    ...
    def encrypt(self, data):
        ...
        if int(t, 16) == 0:
            ...
            if self.mode:
                return bytes.fromhex('%s%s%s' % (C1, C3, C2)), C1 # bytes.fromhex('%s' % C1)
            else:
                return bytes.fromhex('%s%s%s' % (C1, C2, C3)), C1 # bytes.fromhex('%s' % C1)

    def decrypt(self, data, T2):
        # 解密函数，data密文（bytes）
        data = data.hex()
        len_2 = 2 * self.para_len
        len_3 = len_2 + 64
    ...
    # 椭圆曲线计算相关代码
    def genrateT1(self, C1):
        _n = int(self.ecc_table['n'], 16)
        # self.d1 = int(func.random_hex(self.para_len), 16)
        d1_re = self.inverse_mod_prime(self.d1, _n)
        if C1 != '0''0':
            return self._kg(d1_re, C1)
        else:
            print("Error: C1 = 0")
            exit()


    def genrateT2(self, T1, d2):
        _n = int(self.ecc_table['n'], 16)
        d2_re = self.inverse_mod_prime(d2, _n)
        return self._kg(d2_re, T1)
